...
class NavsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create nav" do
    assert_difference("Nav.count") do
      post navs_url, params: { nav: { active: @nav.active, code: @nav.code, language_id: @nav.language_id, name: @nav.name } }
    end
  ...
  test "should update nav" do
    patch nav_url(@nav), params: { nav: { active: @nav.active, code: @nav.code, language_id: @nav.language_id, name: @nav.name } }
